...
async function exportSelectionAsImage(node: SceneNode): Promise<string | null> {
  try {
    ...
    const bytes = await node.exportAsync({
      format: 'PNG',
      constraint: { type: 'SCALE', value: 1 } // 1倍の解像度でエクスポート
    });
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'enter-selection-mode') {
    // 選択モードに入る
    figma.notify('レイヤーを選択してください');
    
    // 選択変更時のイベントを即座に送信するように設定
    const sendSelectionUpdate = () => {
      const selectionInfo = figma.currentPage.selection.map(node => ({
        id: node.id,
        name: node.name,
        type: node.type
      }));
    ...
    try {
      const settings = {
        ...
        autoSizeEnabled: await figma.clientStorage.getAsync('auto-size-enabled') !== false,
        // コーディング設定を追加
        directCodeOutput: await figma.clientStorage.getAsync('direct-code-output') || false,
        addCodePrefix: await figma.clientStorage.getAsync('add-code-prefix') !== false,
        codePrefix: await figma.clientStorage.getAsync('code-prefix') || 'component',
        useBEMNotation: await figma.clientStorage.getAsync('use-bem-notation') || false,
        mergeCssInBatch: await figma.clientStorage.getAsync('merge-css-in-batch') !== false
      };
      figma.ui.postMessage({
      ...
      figma.ui.postMessage({
        ...
        settings: {
          ...
          // コーディング設定のデフォルト値
          directCodeOutput: false,
          addCodePrefix: true,
          codePrefix: 'component',
          useBEMNotation: false,
          mergeCssInBatch: true
        }
      });
    }
    ...
    try {
      ...
      if (autoSizeEnabled !== undefined) {
        await figma.clientStorage.setAsync('auto-size-enabled', autoSizeEnabled);
      }
      
      // コーディング設定を保存
      if (directCodeOutput !== undefined) {
        await figma.clientStorage.setAsync('direct-code-output', directCodeOutput);
      }
      
      if (addCodePrefix !== undefined) {
        await figma.clientStorage.setAsync('add-code-prefix', addCodePrefix);
      }
      
      if (codePrefix !== undefined) {
        await figma.clientStorage.setAsync('code-prefix', codePrefix);
      }
      
      if (useBEMNotation !== undefined) {
        await figma.clientStorage.setAsync('use-bem-notation', useBEMNotation);
      }
      
      if (mergeCssInBatch !== undefined) {
        await figma.clientStorage.setAsync('merge-css-in-batch', mergeCssInBatch);
      }
    } catch (error) {
    ...
    try {
      ...
      console.log(`エクスポートリクエスト: ${nodeIds.length}件のノードID`);
      
      // 選択中のノードをフォールバックとして保持
      const selectedNodes = figma.currentPage.selection as SceneNode[];
      console.log(`現在の選択: ${selectedNodes.length}件のノード`);
      
      // nodeInfoMapから設定情報を取得
      const nodeInfoMap = msg.nodeInfoMap || {};
      const prioritizeIndividualNodes = msg.prioritizeIndividualNodes === true;
      
      // 選択の処理をスキップするかどうか
      let skipSelectionExport = false;
      
      // 設定情報に選択要素のスキップフラグがある場合
      if (nodeInfoMap['__selection__'] && nodeInfoMap['__selection__'].skipSelectionExport === true) {
        skipSelectionExport = true;
        console.log('選択全体のエクスポートをスキップします（個別ノードが指定されているため）');
      }
      
      // エクスポート結果
      const resultImages: Array<{ id: string, name: string, data: string }> = [];
      
      // 指定IDのノードを処理（特定IDのノードを優先的に処理）
      if (nodeIds.length > 0) {
        console.log(`${nodeIds.length}件の指定IDノードを処理します`);
        ...
        for (const nodeId of nodeIds) {
          ...
          if (node && 'exportAsync' in node) {
            try {
              ...
              // エクスポート形式を決定
              const exportType = nodeInfo.exportType || nodeInfo.type || 'png';
              const exportScale = nodeInfo.exportScale || 1;
              let proposedName = nodeInfo.proposedName;
              
              // Figmaエクスポート設定の生成
              let format: 'PNG' | 'JPG' | 'SVG' | 'PDF';
              ...
              console.log(`ノード "${node.name}" (${nodeId}) を ${format} 形式、${exportScale}x でエクスポート`);
              ...
              if (proposedName) {
                // 提案名を使用（ただし安全な文字列に変換）
                safeName = proposedName.replace(/[^\w\s\-\.]/gi, '_').trim();
              } else {
                // ノード名から安全なファイル名を生成
                safeName = node.name.replace(/[^\w\s]/gi, '_').trim() || 'image';
              }
              
              // 拡張子を含むファイル名
              const extension = exportType.toLowerCase().replace('jpeg', 'jpg');
              const filename = `${safeName}.${extension}`;
              
              console.log(`エクスポート成功: "${filename}"`);
              ...
              console.error(`ノード "${node.name}" (${nodeId}) のエクスポートエラー:`, exportError);
            }
          } else {
            console.warn(`ノードID ${nodeId} が見つからないかエクスポート不可: ${node ? node.type : 'not found'}`);
          }
        }
      }
      
      // 個別IDの処理が完了したか、指定IDがない場合で、選択全体のエクスポートをスキップしない場合
      if (!skipSelectionExport && (resultImages.length === 0 || !prioritizeIndividualNodes)) {
        // 従来の処理を保持（選択全体から画像ノードを検出してエクスポート）
        console.log('選択全体から画像ノードを検出してエクスポート (nodeInfoMap を使用)');
        // exportImageNodesFromLayers に nodeInfoMap を渡すように修正
        const images = await exportImageNodesFromLayers(selectedNodes, msg.nodeInfoMap || {});
        resultImages.push(...images.map(img => ({
          id: img.id,
          name: img.name,
          data: img.data
        })));
  ...
  } else if (msg.type === 'detect-image-nodes') {
    try {
      // 指定されたノードIDを使用してノードを取得 (なければ現在の選択)
      let nodes: SceneNode[] = [];
      
      if (msg.nodeIds && msg.nodeIds.length > 0) {
        console.log(`Processing ${msg.nodeIds.length} node IDs for image detection`);
        nodes = msg.nodeIds
          .map((id: string) => figma.getNodeById(id))
          .filter((node: BaseNode | null): node is SceneNode => node !== null && 'type' in node) as SceneNode[];
        console.log(`Found ${nodes.length} valid nodes for detection`);
      } else {
        nodes = figma.currentPage.selection as SceneNode[];
        console.log(`Using ${nodes.length} selected nodes for detection`);
      }
      
      if (nodes.length === 0) {
        console.warn('No valid nodes found for image detection');
        figma.ui.postMessage({
          type: 'detect-image-nodes-result',
          success: false,
      ...
      // 画像ノードを検索
      console.log('Starting image node detection process');
      const imageNodesPromises = nodes.map(node => findImageNodesInLayer(node));
      const imageNodesArrays = await Promise.all(imageNodesPromises);
      
      // 結果を平坦化
      const imageNodes = imageNodesArrays.flat();
      console.log(`Total image nodes detected: ${imageNodes.length}`);
      
      // 検出結果を返す
      figma.ui.postMessage({
        type: 'detect-image-nodes-result',
        success: true,
        imageNodes: imageNodes.map(item => ({
          id: item.node.id,
          name: item.node.name,
          type: item.node.type,
          reason: item.reason,
          size: 'width' in item.node && 'height' in item.node ? {
      ...
      console.error('Error in detect-image-nodes:', error);
      figma.ui.postMessage({
        type: 'detect-image-nodes-result',
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
async function findImageNodesInLayer(node: SceneNode): Promise<Array<{node: SceneNode, reason: string}>> {
  ...
  if (node.type === 'RECTANGLE' || node.type === 'ELLIPSE' || node.type === 'POLYGON' || node.type === 'STAR' || node.type === 'VECTOR' || node.type === 'FRAME') {
    ...
    if (hasImageFill) {
      console.log(`Found image fill in node: ${node.name}`);
      imageNodes.push({
        node,
        reason: 'IMAGE_FILL'
      });
  ...
  if ('type' in node && (node as any).type === 'IMAGE') {
    console.log(`Found direct image node: ${node.name}`);
    imageNodes.push({
      node,
      reason: 'IMAGE_NODE'
    });
  ...
  // 子要素を持つノードなら再帰的に処理
  if ('children' in node) {
    // 子要素が多すぎる場合は処理を制限（パフォーマンス向上のため）
    const maxChildrenToProcess = 20;
    const childrenToProcess = node.children.slice(0, maxChildrenToProcess);
    console.log(`Processing ${childrenToProcess.length}/${node.children.length} children of node: ${node.name}`);
    ...
    // 処理した子要素数と全子要素数が異なる場合はログを出力
    if (childrenToProcess.length < node.children.length) {
      console.log(`Note: Skipped processing ${node.children.length - childrenToProcess.length} children for performance reasons`);
    }
  }
...
// 第2引数に nodeInfoMap を追加
async function exportImageNodesFromLayers(nodes: readonly SceneNode[], nodeInfoMap: Record<string, any>): Promise<Array<{id: string, name: string, data: string, nodeId: string}>> {
  const allImages: Array<{id: string, name: string, data: string, nodeId: string}> = [];

  // ノード情報マップを取得（存在する場合） -> 引数で受け取るため削除
  // const nodeInfoMapData = figma.currentPage.getPluginData('nodeInfoMap');
  // let formatInfoMap: Record<string, {type: string, exportScale: number}> = {};
  // try {
  //   // プラグインデータから出力形式情報を取得（存在する場合）
  //   if (nodeInfoMapData) {
  //     formatInfoMap = JSON.parse(nodeInfoMapData);
  //   }
  // } catch (e) {
  //   console.error('Failed to parse nodeInfoMap:', e);
  // }
  // console.log('Exporting with format info:', formatInfoMap); // formatInfoMap -> nodeInfoMap に変更してデバッグログを残す
  console.log('Exporting with format info from argument:', nodeInfoMap);

  for (const node of nodes) {
    try {
      // レイヤー内の画像を検索
      const imageNodes = await findImageNodesInLayer(node);
      console.log(`Found ${imageNodes.length} image nodes in ${node.name}`);

      if (imageNodes.length === 0) {
        // 画像が見つからない場合はレイヤー自体をエクスポート（従来の動作）
        // ノードのエクスポート形式情報を取得 - formatInfoMap -> nodeInfoMap を使用
        const nodeFormat = nodeInfoMap[node.id] || { type: 'png', exportScale: 2 };

        // 形式をFigmaの形式名に変換
        const formatType = getExportFormat(nodeFormat.type || 'png'); // type がない場合のフォールバックを追加
        const scale = nodeFormat.exportScale || 2;
        ...
        if (formatType === 'SVG') {
          ...
          console.log(`Exporting node ${node.name} as SVG`);
          ...
          try {
            ...
            // Base64エンコード
            const base64 = figma.base64Encode(bytes);
            const mimeType = getMimeType(nodeFormat.type || 'svg'); // type がない場合のフォールバックを追加
            const data = `data:${mimeType};base64,${base64}`;

            console.log(`Exported ${node.name} successfully as SVG, data length: ${data.length}`);

            // ファイル名生成 - proposedName を考慮
            const proposedName = nodeFormat.proposedName;
            let safeName;
            if (proposedName) {
              safeName = proposedName.replace(/[^\\w\\s\\-\\.]/gi, '_').trim();
            } else {
            ...
            const extension = (nodeFormat.type || 'svg').toLowerCase().replace('jpeg', 'jpg'); // type がない場合のフォールバック
            ...
            console.error(`SVG export error for node ${node.name}:`, exportError);
          }
        } else {
          // 非SVG形式の場合は従来通りの処理
          exportSettings = {
          ...
          console.log(`Exporting node ${node.name} as ${formatType} with scale ${scale}x`);
          ...
          try {
            ...
            // Base64エンコード
            const base64 = figma.base64Encode(bytes);
            const mimeType = getMimeType(nodeFormat.type || 'png'); // type がない場合のフォールバックを追加
            const data = `data:${mimeType};base64,${base64}`;

            console.log(`Exported ${node.name} successfully, data length: ${data.length}`);
            ...
            const extension = (nodeFormat.type || 'png').toLowerCase().replace('jpeg', 'jpg'); // type がない場合のフォールバック
        ...
        for (const [index, imageNode] of imageNodes.entries()) {
          ...
          // ノードのエクスポート形式情報を取得 - formatInfoMap -> nodeInfoMap を使用
          // 元のノードIDがnodeInfoMapに存在するかをまず確認
          const nodeFormat = nodeInfoMap[exportNode.id] ||
                            nodeInfoMap[node.id] ||
                            { type: 'png', exportScale: 2 };
          ...
          if (formatType === 'SVG') {
            ...
            console.log(`Exporting image node ${exportNode.name} as SVG`);
          } else {
            // 非SVG形式の場合
            exportSettings = {
              format: formatType,
            ...
            console.log(`Exporting image node ${exportNode.name} as ${formatType} with scale ${scale}x`);
          }
          ...
          try {
            ...
            console.log(`Exported ${exportNode.name} successfully, data length: ${data.length}`);

            // ノード名からファイル名を生成（複数ある場合はインデックスを追加） - proposedName を考慮
            const proposedName = nodeFormat.proposedName;
            let safeName;
            if (proposedName) {
              safeName = proposedName.replace(/[^\\w\\s\\-\\.]/gi, '_').trim();
            } else {
              safeName = exportNode.name.replace(/[^\\w\\s]/gi, '_').trim();
            }
            ...
            console.error(`Export error for image node ${exportNode.name}:`, exportError);
          }
        }
      }
    } catch (error) {
...
// AIが提案したunknown-icon-Xなどの未知のIDを検出
const detectUnknownIds = (nodeIds: string[]): string[] => {
  return nodeIds.filter(id => 
    id.startsWith('unknown-') || 
    id.startsWith('icon-') && !figma.getNodeById(id)
  );
};

// 形式をFigmaのexportAsync用の形式に変換するヘルパー関数
function getExportFormat(type: string): 'PNG' | 'JPG' | 'SVG' | 'PDF' {
